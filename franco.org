...
"el" ( or "al") in compliant Franco is attached to the following word using hyphens, e.g. /el-walad/. for consistency please stick to this writing, i.e. don't write /el walad/ or /elwalad/.
some words start with letters (sun letters) that assimilate the "l" in the definite article so that l is removed and shadda is added to the letter. for example some words like /el naharda/ are often pronounced as /en-naharda/, whether or not this increases readability or not is very dependent on the context and the word itself. as in the usual case if you choose to assimilate the "l" in certain word you should write it like /en-naharda/, not /en naharda/ or /ennaharda/.
...
those prepositions inherit the same rules of attaching from the definite article, by that we mean for example if you want to assimilate the l of the definite article in "fel-konAfa it should be written as "fek-konAfa".

ma can optionaly be attached directly to the following word, e.g. 3ala mAgy, 3omry mansa, ana mabein nArein.
...
* Don't start with "2"
since in english e,o,a in the beginning of words contain "2" in their pronounciation, we can borrow this property.
